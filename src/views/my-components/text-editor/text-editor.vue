<template>
     <div style='position: relative;height:100%;padding:10px;'>
      ...
      <Table :columns="historyColumns" :data="historyData" :loading='SpinType' ></Table>
       ...
       <Row type="flex" justify="center" style="margin-top:30px;">
         <Col span="22" style="text-align:right;">
          <DatePicker type="datetime"  placeholder="请选择日期时间" @on-change="onHandChange" @on-open-change='changepage' @on-ok="handOk" style="width: 200px" :value="value1" format="yyyy年MM月dd日"></DatePicker>
         </Col>
      ...
      <Modal
        ...
        @on-ok="modelOk"
        @on-cancel="modelOk">
        <p v-for="item in resModalList">库名：{{item.dbName}},表名：{{item.tableName}},权限：{{item.operate}}, 权限修改结果：{{item.secrityStatus==1?'成功':'失败'}} </p>
...
export default {
  ...
  created() {
      ...
      let default_date=year+':'+month+':'+day+':'+hour+':'+minute+':'+second;
      this.value1=default_date;
    this.initEcharts();
    this.initEcharts();
    this.initList();
  },
